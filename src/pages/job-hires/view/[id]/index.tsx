...
import { getJobHireById } from 'apiSdk/job-hires';
...
function JobHireViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<JobHireInterface>(
    () => (id ? `/job-hires/${id}` : null),
    () =>
      getJobHireById(id, {
        relations: ['user', 'blog', 'project'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Job Hire Detail View
          </Text>
          {hasAccess('job_hire', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/job-hires/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('blog', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/blogs/create?job_hire_id=${data?.id}`}>
              ...
              {hasAccess('project', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/projects/create?job_hire_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
