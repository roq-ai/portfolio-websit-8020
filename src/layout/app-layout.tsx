...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['visitor', 'blog-subscriber', 'project-viewer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<JobHireInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/job-hires` : null),
    () => getJobHires({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Job Hire',
      path: isTenantUser ? `/job-hires/view/${data?.[0]?.id}` : '/job-hires',
      entity: 'job_hire',
